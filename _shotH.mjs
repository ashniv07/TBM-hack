...
// force our own rebuild so we know the graph exists right now, regardless of concurrent external changes
const rebuildLabel = await page.evaluate(() => {
  const btn = Array.from(document.querySelectorAll("button")).find(b => b.textContent?.includes("Rebuild"));
  return btn?.textContent;
});
console.log("rebuild button:", rebuildLabel);
await page.evaluate(() => {
  const btn = Array.from(document.querySelectorAll("button")).find(b => b.textContent?.includes("Rebuild"));
  btn?.click();
});
// wait for rebuild to finish (poll for success banner or timeout)
await page.waitForFunction(
  () => document.body.textContent?.includes("Model rebuilt"),
  { timeout: 60000 }
).catch(() => console.log("rebuild banner wait timed out, continuing anyway"));
await new Promise(r => setTimeout(r, 800));

await clickButtonText("Graph View");
await new Promise(r => setTimeout(r, 1200));
await page.screenshot({ path: "_shotH_overview.png" });

const info = await page.evaluate(() => {
  const svg = document.querySelector("svg.graph-viz");
  if (!svg) return { noSvg: true, bodyText: document.body.textContent?.slice(0, 300) };
